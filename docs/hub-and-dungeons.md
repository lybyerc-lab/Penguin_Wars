...
- **Seams** for an optional boss phase, run modifiers, character selection, campaign and save state, and multiple regions. These are interfaces and small data resources, not the features behind them; [architecture.md](architecture.md) says how to extend each one.
...
- More caves, secrets and puzzles. One cave exists. A second is an entry in a region's cave list; a second region is one resource. The town's building layout is still a code table because one town exists.
- Boss content. The phase, placement, scaling and reward path are here and tested with a stub; the bosses themselves belong to the gameplay lane.
