class_name BossHUD
extends CanvasLayer
## Name and health for the current boss. Idle and invisible when no boss is
## in the room, so both runnable scenes can carry one unconditionally.

var encounter: EncounterDirector
## Used only to clear the player cards, which are taller for a large party.
...
func _on_boss_started(boss: ArenaBoss) -> void:
...
func _process(_delta: float) -> void:
	...
	_name.text = boss.definition.display_name + (" · ENRAGED" if brain != null and brain.enraged else "")
