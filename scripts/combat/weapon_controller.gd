...
func _physics_process(delta: float) -> void:
	...
	for candidate: Node in targets:
		...
		if distance >= definition.reach * definition.reach:
			continue
		# Threats always win targeting priority over supply props.
		var score: float = distance + (definition.reach * definition.reach if enemy.is_in_group("breakables") else 0.0)
		if score < best:
			best = score
			target = enemy
	if target != null:
...
func _hit(enemy: Node2D) -> void:
	...
	(enemy.get_node("Health") as Health).take_damage(DamageEvent.new(definition.damage + damage_bonus, wielder.identity.player_id, push))
