...
## Every attack is preceded by a locked, drawn warning, like the charger and
## thrower. Damage taken during a windup cannot cancel it, so heavy knockback
## cannot stun-lock the fight.

enum State { STALK, WINDUP, RUSH, RECOVER }
enum Attack { RUSH, VOLLEY, SLAM }

## How far the charge lane reaches, for both the warning and the reader.
...
func movement(enemy: ArenaEnemy, target: PenguinPlayer, delta: float) -> Vector2:
	...
	if boss == null or boss.definition == null:
		return Vector2.ZERO
...
func _slam(boss: ArenaBoss) -> void:
...
## Contact only lands during the rush itself, never the warning or recovery.
